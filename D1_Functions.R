  ...
  ###
   ...
   if(pattern == "stars"){
     ...
     skin_1 <- naked_0 %>% 
       ...
       mutate(color = case_when(
         Chart == " Original" ~ "#CCCCCC",
         p_dist <  dot & abs(x-x_mid) <= star_width ~ sel_color[2],
         p_dist <  dot & abs(y-y_mid) <= star_width ~ sel_color[2],
         p_dist <  (3/4)*dot & abs((x-x_mid) + (y-y_mid)) <= star_width ~ sel_color[2],
         p_dist <  (3/4)*dot & abs((x-x_mid) - (y-y_mid)) <= star_width ~ sel_color[2],
         TRUE ~ sel_color[1]
       )) 
     
     #Save pattern details
     pattern_specs <- list(pattern = "stars", 
                           group_radius = group_radius, 
                           dot_sizes = dot_sizes, dot_radius = dot_radius)
   }
...
#Plot the datasaur
plot_datasaur <- function(skin_datasaur0, version_num){
  ...
  png(filename = plot.file,
    ...
    grid.arrange(
        ...
        top = textGrob(paste0(datasaur_name), 
                       ...
                       x = 0, just = "left"),
        bottom = textGrob(paste0("#", tweet_number, " | ", "@Datasaurs v", version_num),
...
text_datasaur <- function(plot_datasaur0){
  ...
  if(sample(c(TRUE, FALSE), 1, prob = c(10, 1))){
    text_hashtags <- paste0(sample(names(hashes_list), 1, prob = hashes_list),
                           ...
                           if(pattern$thanksgiving){ "#HappyThanksgiving"}, 
  ...
  if(as.character(info[1, "Twitter"]) != "" && !is.na(info[1, "Twitter"]) &&
     sample(c(TRUE, FALSE), 1, prob = c(1, 12))){
